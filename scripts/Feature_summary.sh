...
"https://$JIRA_DOMAIN/rest/agile/1.0/board/$BOARD_ID/issue?jql=issuetype=Story%20AND%20fixVersion=%22$FIX_VERSION%22%20AND%20'Feature%20Link'%20is%20not%20EMPTY&fields=customfield_10091,status,labels,description")
...
for feature_url in "${!feature_map[@]}"; do
  ...
  html_table="${html_table}<tr style='border:1px solid #000;'><td style='border:1px solid #000;'>${feature_url}</td><td style='border:1px solid #000;'>${feature_summary}</td><td style='border:1px solid #000;'>${completion_percentage}%</td></tr>"
done
...
echo "feature_html_table=${encoded_html}" >> $GITHUB_ENV
